...
#if defined(NDEBUG)
#error "APR Coin cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < Params().LAST_POW_BLOCK1() && nHeight > 0)
            return 50000 * COIN;
    }

    if (nHeight < Params().LAST_POW_BLOCK1())
        nSubsidy = 15000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < Params().LAST_POW_BLOCK1() || blockValue == 0)
        return 0;
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
        ...
        if (!tx.IsCoinStake()) {
            int nBestHeight = GetHeight();
            if (nBestHeight > 316050) {
...
bool IsProofOfWorkPeriod(int nHeight) {
    if (nHeight <= Params().LAST_POW_BLOCK1()) {
        return true;
    }
    if (nHeight > Params().LAST_POW_BLOCK2() - Params().LAST_POW_BLOCK1() && nHeight <= Params().LAST_POW_BLOCK2()) {
        return true;
    }
    return false;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (IsProofOfWorkPeriod(pindex->nHeight) && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active"),
            REJECT_INVALID, "PoS-early");

    if (!IsProofOfWorkPeriod(pindex->nHeight) && block.IsProofOfWork())
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.IsProofOfWork() && IsProofOfWorkPeriod(pindexPrev->nHeight + 1)) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // APR: We use certain sporks during IBD, so check to see if they are
