...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AprCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("AprCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("AprCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aprcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            if (IsProofOfWorkPeriod(chainActive.Tip()->nHeight + 1)) {
                MilliSleep(5000);
                continue;
            }
        ...
        LogPrintf("Running AprCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        if (IsProofOfWorkPeriod(pindexPrev->nHeight + 1)) {
            hashTarget = Params().ProofOfWorkLimit() >> 23;
        }
        while (true) {
